...
Composition features are position-blind. Two peptides with identical amino acid
composition but different Amino acids order are treated as identical.
...
Rare modifications are underpowered. Acetylation and multiple oxidation events show
interesting trends, but sample sizes are too small for strong conclusions.
...
Add position-aware sequence features to investigate effects such as proline placement.

Investigate extreme peptides to distinguish unusual chemistry from measurement issues.

Collect more examples of rare modifications to test whether their effects are
consistent.

If prediction became the goal, evaluate sequence-aware models using a held-out test
set.
...
Retention coefficients retrieved from ExPASy ProtScale.